...
protected:
    void _create_dynamic(lv_obj_t* parent) override {
        ...
        lv_obj_set_style_text_font(_lb_symbol, &lv_font_montserrat_28, 0);
        lv_obj_set_style_text_color(_lb_symbol, lv_color_hex(0xEE7D30), 0);
        lv_obj_align(_lb_symbol, LV_ALIGN_CENTER, 0, -45);

        _lb_price = lv_label_create(parent);
        lv_label_set_text(_lb_price, "$--");
        lv_obj_set_style_text_font(_lb_price, &lv_font_montserrat_28, 0);
        ...
        lv_obj_set_style_text_font(_lb_change, &lv_font_montserrat_20, 0);
        lv_obj_set_style_text_color(_lb_change, lv_color_hex(0xA9A9A9), 0);
        lv_obj_align(_lb_change, LV_ALIGN_CENTER, 0, 40);
    }
};
