...
static void config_timeout_monitor_thread_entry(void *args){
    ...
    uint16_t timeout = 0;
    g_board.info.connection.wifi.status_param.config_timeout = CONFIG_TIMEOUT;
    while(true){
        if (WL_CONNECTED == g_board.info.connection.wifi.status_param.status) {
            break;
        }

        if(g_board.info.connection.client_connected == false){
            if(timeout++ >= CONFIG_TIMEOUT){
                LOG_W("WiFi configuration timeout, rebooting...");
                delay(1000);
                ESP.restart();
            }
            g_board.info.connection.wifi.status_param.config_timeout = CONFIG_TIMEOUT - timeout;
        }
        delay(1000);
    }
...
void wifi_connect_thread_entry(void *args){
    ...
    WiFi.setHostname(g_board.info.base.hostname.c_str());
