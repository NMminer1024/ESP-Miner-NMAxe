...
static void ui_ota_page_update(){
  ...
  lv_label_set_text(label_file, g_board.status.ota.filename.c_str());
...
static void ui_hr_healthy_page_update(miner_status_t *miner_status){
  uint16_t SCALE = (g_board.ui.hr_dist_page.max_x_hr / g_board.ui.hr_dist_page.max_x_bars);
  ...
  if(first_time){
    ...
    lv_chart_set_range(chart, LV_CHART_AXIS_PRIMARY_X, 0, g_board.ui.hr_dist_page.max_x_bars - 1); 
    ...
    lv_chart_set_point_count(chart, g_board.ui.hr_dist_page.max_x_bars);
    lv_chart_set_all_value(chart, series, 0);
    lv_chart_set_axis_tick(chart, LV_CHART_AXIS_PRIMARY_X, 1, 1, g_board.ui.hr_dist_page.max_x_bars, 1, true, 25);
  ...
  if (counts == NULL) {
    counts = (uint64_t *)malloc(g_board.ui.hr_dist_page.max_x_bars * sizeof(uint64_t));
    memset(counts, 0, g_board.ui.hr_dist_page.max_x_bars * sizeof(uint64_t));
  }
  int index = last_hashrate/1000/1000/1000 / SCALE; // Convert to GH/s and scale
  index = (index >= g_board.ui.hr_dist_page.max_x_bars) ? g_board.ui.hr_dist_page.max_x_bars - 1 : index;
  counts[index]++;
  g_board.ui.hr_dist_page.times++;
  for (int i = 0; i < g_board.ui.hr_dist_page.max_x_bars; i++) {
    uint8_t y = (uint8_t)(100*(float)counts[i] / (float)g_board.ui.hr_dist_page.times);
    lv_chart_set_value_by_id(chart, series, i, y);
    g_board.ui.hr_dist_page.dist_map[i] = y;// Update the global distribution map
  }
  // time cost of this feature
  static uint64_t start = millis();
  g_board.ui.hr_dist_page.dura = (millis() - start) / 1000;
  ...
  lv_label_set_text_fmt(lb_ds_hr_unit, "%s", hr_unit.c_str());
  //time cost
  lv_label_set_text_fmt(lb_hr_health_duration,"Sample: %s", String(String(g_board.ui.hr_dist_page.times) + "t/"+ String(g_board.ui.hr_dist_page.dura) + "s").c_str());
}
