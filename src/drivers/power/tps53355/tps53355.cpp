...
void NMAxePowerClass::set_vcore_status(power_state_t state){
    if(state == PWR_OFF){
...
//     baord->power->init();
//     delay(500);
//     while (baord->power->get_vbus() < VBUS_MIN_VOLTAGE){
//         LOG_W("Vbus is %.2fV , at least %.2fV required, waiting for power setup...", baord->power->get_vbus()/1000.0, VBUS_MIN_VOLTAGE/1000.0);
//         delay(1000);
//     }
