...
void PageDashboard320x240::create(lv_obj_t* parent) {
    ...
    _H = 240;
    _parent = parent;
    _show_miner_diff = true;
    _show_vcore_temp_ring = true;
    _miner_img_dsc = &logo_miner_nmqaxepp_70_70;
    _img_miner_x = 0;
    _img_miner_y = 55;
    _lb_hr_x = 95;
    _lb_hr_y = -4;
...
void PageDashboard320x240::_create_dynamic(lv_obj_t* parent) {
    ...
    lv_obj_set_style_text_font(_lb_hr, &ds_digib_font_56, LV_PART_MAIN);
    ...
    _lb_diff = lv_label_create(parent);
    lv_obj_set_width(_lb_diff, _miner_img_dsc->header.w);
    lv_label_set_text(_lb_diff, "0.000");
    lv_obj_set_style_text_font(_lb_diff, &Inconsolata_16, LV_PART_MAIN);
    lv_obj_set_style_text_align(_lb_diff, LV_TEXT_ALIGN_LEFT, LV_PART_MAIN);
    lv_obj_set_style_text_color(_lb_diff, lv_color_hex(0xFFFFFF), LV_PART_MAIN);
    lv_label_set_long_mode(_lb_diff, LV_LABEL_LONG_WRAP);
    lv_obj_align(_lb_diff, LV_ALIGN_CENTER,
                 _img_miner_x + _miner_img_dsc->header.w,
                 _img_miner_y + _miner_img_dsc->header.h / 2 + 10);

    const uint8_t arc_r = 30;
    const uint8_t arc_line_width = 8;
    _ring_oc.cfg = {
        0, 8, arc_r, arc_line_width, 0, 0, _ring_angle_full,
    ...
    _ring_pwr.cfg = {
        (lv_coord_t)(2 * arc_r + 10), 8, arc_r, arc_line_width, 0, 0, _ring_angle_full,
        lv_color_hex(0xC0C0C0), lv_color_hex(0x0080FF),
        " ", &lv_font_montserrat_14, lv_color_hex(0xFFFFFF),
    ...
    _ring_asic_temp.cfg = {
        227, 50, (lv_coord_t)(3 * arc_r / 2), (lv_coord_t)(arc_line_width * 2), 0, 0, _ring_angle_full,
        lv_color_hex(0xC0C0C0), lv_color_hex(0x0080FF),
        " ", &lv_font_montserrat_20, lv_color_hex(0xFFFFFF),
        "ASIC Temp", &lv_font_montserrat_14, lv_color_hex(0xD3D3D3)
    };
    _ring_vcore_temp.cfg = {
        132, 50, (lv_coord_t)(3 * arc_r / 2), (lv_coord_t)(arc_line_width * 2), 0, 0, _ring_angle_full,
        lv_color_hex(0xC0C0C0), lv_color_hex(0x0080FF),
        " ", &lv_font_montserrat_20, lv_color_hex(0xFFFFFF),
        "Vc Temp", &lv_font_montserrat_14, lv_color_hex(0xD3D3D3)
    };
}
